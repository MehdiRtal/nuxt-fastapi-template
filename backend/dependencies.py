...
def get_current_user(db_session: DBSession, session_id: str = Cookie(None)):
    try:
        payload = jwt.decode(session_id, settings.JWT_SECRET, algorithms=settings.JWT_ALGORITHM)
    except JWTError:
        raise HTTPException(status_code=401, detail="Invalid session_id")
    else:
        db_user = db_session.get(User, payload["user_id"])
        return db_user

CurrentUser = Annotated[User, Depends(get_current_user)]

def get_verify_user(db_session: DBSession, verification_sid: str, verification_code: str):
