...
class UsersService:
    ...
    async def change_current_user_password(self, current_user: User, current_password: str, new_password: str) -> UserRead:
    ...
    async def unlink_current_user_google(self, current_user: User) -> UserRead:
    ...
    async def get_current_user_items(self, current_user: User, limit: int = 100, offset: int = 0) -> list[ItemRead]:
        db_items = await self.items_repository.get_by_user_id(current_user.id, limit, offset)
        return db_items

    async def get_current_user_item(self, current_user: User, item_id: int) -> ItemRead:
        db_item = await self.items_repository.get_by_id_by_user_id(current_user.id, item_id)
        return db_item

    async def add_current_user_item(self, current_user: User, item: ItemCreate) -> ItemRead:
        item.user_id = current_user.id
        db_item = await self.items_repository.add(item)
        return db_item

    async def update_current_user_item(self, current_user: User, item_id: int, item: ItemUpdate) -> ItemRead:
        db_item = await self.items_repository.update_by_id_by_user_id(current_user.id, item_id, item)
        return db_item

    async def delete_current_user_item(self, current_user: User, item_id: int) -> ItemRead:
        db_item = await self.items_repository.delete_by_id_by_user_id(current_user.id, item_id)
        return db_item

    async def get_current_user(self, current_user: User) -> UserRead:
        return current_user

    async def update_current_user(self, current_user: User, user: UserUpdate) -> UserRead:
        for key, value in user.model_dump(exclude_unset=True).items():
            setattr(current_user, key, value)
        current_user = await self.users_repository.update(current_user)
        return current_user

    async def delete_current_user(self, current_user: User) -> UserRead:
